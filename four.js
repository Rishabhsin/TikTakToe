...
ctx.moveTo(area * 0.25, 0);
ctx.lineTo(area * 0.25, area);
ctx.moveTo(area * 0.5, 0);
ctx.lineTo(area * 0.5, area);
ctx.moveTo(area * 0.75, 0);
ctx.lineTo(area * 0.75, area);
ctx.moveTo(0, area * 0.25);
ctx.lineTo(area, area * 0.25);
ctx.moveTo(0, area * 0.5);
ctx.lineTo(area, area * 0.5);
ctx.moveTo(0, area * 0.75);
ctx.lineTo(area, area * 0.75);
ctx.lineWidth = 10;
ctx.stroke();
...
function check(move) {
  ...
  if (!winner) {
    ...
    function aiFindSpot() {
      ...
      if (
        indexOfCleanWinCombos[indexes[randomNumber]][1][secondRandom] == "ai"
      ) {
        aiFindSpot();
      } else {
